...
def connection_check():
    print("checking osrm connection...")
    result = nearest((1, 1))
    if "code" not in result or "Ok" not in result["code"]:
        raise ValueError("could not connect to osrm " + json.dumps(RequestConfig))
    print("osrm connection check succeeded")
