...
export class TrucksMapComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.initialize_map();
    this.refresh = false; // todo: toggle for auto-refresh
  ...
  async schedule_refresh() {
    while (this.refresh) {
      await new Promise((r) => setTimeout(r, REFRESH_TIMEOUT_MS));
      this.initialize_map();
    }
  }
    ...
    this.osrmRouteService.retrieveRoute(locations).subscribe(
      (osrmRoute) => {
        if (osrmRoute.code != 'Ok') {
          console.log(
            `osrm request failed: ${locations.toString()} with code: ${
              osrmRoute.code
            }`
          );
  ...
  onTourClick(e: LeafletEvent): void {
    const context: TourLocationsClickContext =
      this as unknown as TourLocationsClickContext;
  ...
  onTruckMarkerDrop(e: LeafletEvent): void {
    const context: TrucksMapComponentDropContext =
      this as unknown as TrucksMapComponentDropContext;
